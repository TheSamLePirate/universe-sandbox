...
import { X, Crosshair, Ban, Trash2, Sun } from 'lucide-react';
...
const InfoPanel: React.FC<InfoPanelProps> = ({ body, onClose, allBodies, isFollowing, onToggleFollow, onDelete, onMakeStar }) => {
  if (!body) return null;
  ...
  return (
    <div 
        className="absolute top-4 right-4 w-80 bg-slate-900/90 backdrop-blur-md border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col z-20 max-h-[90vh]"
    ...
    >
      ...
      <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
        <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-white truncate max-w-[150px]">{body.name}</h2>
        </div>
        <div className="flex items-center gap-2">
             {!body.isStar && (
                <button 
            ...
            >
                {isFollowing ? <Ban size={16} /> : <Crosshair size={16} />}
            </button>
